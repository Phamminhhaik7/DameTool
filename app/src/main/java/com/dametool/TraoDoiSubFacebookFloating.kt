...
class TraoDoiSubFacebookFloating : Service() {
    ...
    override fun onCreate() {
        ...
        autoLike.setOnCheckedChangeListener { _, isChecked ->
            autolike = isChecked
            val click = Intent("FIND_FOLLOWFACEBOOK")
            click.putExtra("type", "FOLLOWFACEBOOK")
        ...
        run.setOnCheckedChangeListener { _, isChecked ->
            ...
            if(runstatus){
                if(loaijob == "Follow"){
                    TraoDoiSubFacebookFollow()
                } else {
                    TraoDoiSubFacebookReaction()
                }
            }
        }
    }
    ...
    private fun TraoDoiSubFacebookReaction() {
        serviceScope.launch {
            ...
            try{
                ...
                while (runstatus) {
                    ...
                    var url = URL("https://traodoisub.com/api/?fields=facebook_reaction&access_token=$token&type=ALL")
                    ...
                    for (element in dataArray) {
                        ...
                        val code = elementObject.get("code").asString
                        val type = elementObject.get("type").asString
                        ...
                        val intent = Intent(Intent.ACTION_VIEW, Uri.parse(httpConn.url.toString()))
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) // Thêm cờ này để bắt đầu Activity từ Service
                        ...
                        if (!runstatus) { return@launch }
                        val click = Intent("FIND_REACTION")
                        click.putExtra("type", type)
                        LocalBroadcastManager.getInstance(applicationContext).sendBroadcast(click)
                        withContext(Dispatchers.Main){ messagetext.text = "Thực hiện click!" }
                        sleep(3000)
                        if (!runstatus) { return@launch }
                        url = URL("https://traodoisub.com/api/coin/?type=facebook_reaction&id=$code&access_token=$token")
    ...
    @SuppressLint("SetTextI18n", "UseSwitchCompatOrMaterialCode")
    private fun TraoDoiSubFacebookFollow() {
        serviceScope.launch {
            val countdowntext = floatingView.findViewById<TextView>(R.id.countdown)
            val messagetext = floatingView.findViewById<TextView>(R.id.messenger)
            val dachay = floatingView.findViewById<TextView>(R.id.dachay)
            val thanhcong = floatingView.findViewById<TextView>(R.id.thanhcong)
            try{
                requests(URL("https://traodoisub.com/api/coin/?type=facebook_follow&id=facebook_api&access_token=$token"))
                while (runstatus) {
                    if (!runstatus) { return@launch}
                    var url = URL("https://traodoisub.com/api/?fields=facebook_follow&access_token=$token&type=ALL")
                    var response = requests(url)
                    println(response.toString())
                    val gson = Gson()
                    var jsonObject = gson.fromJson(response.toString(), JsonObject::class.java)
                    ...
                    for (element in dataArray) {
                        ...
                        val intent = Intent(Intent.ACTION_VIEW, Uri.parse("fb://profile/$id"))
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) // Thêm cờ này để bắt đầu Activity từ Service
                        startActivity(intent)
                        withContext(Dispatchers.Main){ messagetext.text = id }
                        for (i in countdown downTo 0 step 1) {
                        ...
                        withContext(Dispatchers.Main){ messagetext.text = "Thực hiện click!" }
                        sleep(2000)
                        url = URL("https://traodoisub.com/api/coin/?type=facebook_follow_cache&id=$code&access_token=$token")
                        response = requests(url)
                        jsonObject = gson.fromJson(response.toString(), JsonObject::class.java)
                        if(jsonObject.get("cache").asInt >=5){
                            println("Đã làm đủ 5 nv")
                            url = URL("https://traodoisub.com/api/coin/?type=facebook_follow&id=facebook_api&access_token=$token")
                            response = requests(url)
                            jsonObject = gson.fromJson(response.toString(), JsonObject::class.java)
                            val data = jsonObject.get("data")
                            val dataObject = gson.fromJson(data.toString(), JsonObject::class.java)
                            val xu = dataObject.get("xu").asString
                            val job_success = dataObject.get("job_success").asString
