...
import com.dametool.databinding.TraodoisubInstragramBinding
...
class TraoDoiSubInstragram : Fragment() {

    private var _binding: TraodoisubInstragramBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = TraodoisubInstragramBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.TraoDoiSubInstragramStart.setOnClickListener {
            maxjob = binding.TraoDoiSubInstragramMaxJob.editText?.text.toString().toIntOrNull() ?: 0
            countdown = binding.TraoDoiSubInstragramCountdown.editText?.text.toString().toIntOrNull() ?: 0
            lifecycleScope.launch(Dispatchers.IO){
                try {
                    var url = URL("https://traodoisub.com/api/?fields=instagram_run&id=$nickdangchay&access_token=$token")
    ...
    private suspend fun getBadges(cookiesMap: Map<String, String>): Boolean {
        val document: Document = Jsoup.connect("https://traodoisub.com/view/chinstagram/")
        ...
        if (badges.isEmpty()) {
            ...
            if (pageText.contains("Đăng nhập", ignoreCase = true)) {
                ...
                withContext(Dispatchers.Main) {
                    val err = listOf("Hãy thêm tài khoản Instragram trước","")
    ...
    fun startFloatingService(context: Context) {
        val intent = Intent(context, TraoDoiSubInstragramFloating::class.java)
