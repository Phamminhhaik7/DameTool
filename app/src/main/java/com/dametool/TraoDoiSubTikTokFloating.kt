...
class TraoDoiSubTikTokFloating : Service() {
    ...
    private fun TraoDoiSubTikTokFollow() {
        serviceScope.launch {
            ...
            try{
               requests(URL("https://traodoisub.com/api/coin/?type=TIKTOK_FOLLOW&id=TIKTOK_FOLLOW_API&access_token=$token"))
                while (runstatus) {
                    if (!runstatus) { return@launch}
                    var url = URL("https://traodoisub.com/api/?fields=tiktok_follow&access_token=$token")
                    ...
                    for (element in dataArray) {
                        ...
                        val uniqueID = elementObject.get("uniqueID").asString
                        val intent = Intent(Intent.ACTION_VIEW, Uri.parse(link))
                        intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
                        startActivity(intent)
                        withContext(Dispatchers.Main){ messagetext.text = uniqueID }
                        for (i in countdown downTo 0 step 1) {
                            withContext(Dispatchers.Main){ countdowntext.text = "Countdown: $i" }
                            sleep(1000)
                            if (!runstatus) { return@launch }
                        ...
                        url = URL("https://traodoisub.com/api/coin/?type=TIKTOK_FOLLOW_CACHE&id=$id&access_token=$token")
                        ...
                        if(jsonObject.get("cache").asInt >=8){
                            ...
                            url = URL("https://traodoisub.com/api/coin/?type=TIKTOK_FOLLOW&id=TIKTOK_FOLLOW_API&access_token=$token")
                            response = requests(url)
                            jsonObject = gson.fromJson(response.toString(), JsonObject::class.java)
                            val data = jsonObject.get("data")
                            ...
                            withContext(Dispatchers.Main){ thanhcong.text = "Thành công: $job_success + $tongxuthem xu" }
                        }
                        demjob += 1
    ...
    @SuppressLint("SetTextI18n", "UseSwitchCompatOrMaterialCode")
    private fun TraoDoiSubTikTokLike() {
        serviceScope.launch {
            val countdowntext = floatingView.findViewById<TextView>(R.id.countdown)
            val messagetext = floatingView.findViewById<TextView>(R.id.messenger)
            val dachay = floatingView.findViewById<TextView>(R.id.dachay)
            val thanhcong = floatingView.findViewById<TextView>(R.id.thanhcong)
            try{
                requests(URL("https://traodoisub.com/api/coin/?type=TIKTOK_LIKE&id=TIKTOK_LIKE_API&access_token=$token"))
                while (runstatus) {
                    if (!runstatus) { return@launch}
                    var url = URL("https://traodoisub.com/api/?fields=tiktok_like&access_token=$token")
                    var response = requests(url)
                    println(response.toString())
                    val gson = Gson()
                    var jsonObject = gson.fromJson(response.toString(), JsonObject::class.java)
                    ...
                    for (element in dataArray) {
                        ...
                        url = URL("https://traodoisub.com/api/coin/?type=TIKTOK_LIKE_CACHE&id=$id&access_token=$token")
                        response = requests(url)
                        jsonObject = gson.fromJson(response.toString(), JsonObject::class.java)
                        if(jsonObject.get("cache").asInt >=8){
                            println("Đã làm đủ 8 nv")
                            url = URL("https://traodoisub.com/api/coin/?type=TIKTOK_LIKE&id=TIKTOK_LIKE_API&access_token=$token")
                            response = requests(url)
                            jsonObject = gson.fromJson(response.toString(), JsonObject::class.java)
                            val data = jsonObject.get("data")
                            val dataObject = gson.fromJson(data.toString(), JsonObject::class.java)
