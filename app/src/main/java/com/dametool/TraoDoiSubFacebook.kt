...
import com.dametool.databinding.TraodoisubFacebookBinding
...
class TraoDoiSubFacebook : Fragment() {

    private var _binding: TraodoisubFacebookBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = TraodoisubFacebookBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.TraoDoiSubTikTokStart.setOnClickListener {
            ...
            lifecycleScope.launch(Dispatchers.IO){
                try {
                    var url = URL("https://traodoisub.com/api/?fields=run&id=$nickdangchay&access_token=$token")
    ...
    private suspend fun getBadges(cookiesMap: Map<String, String>): Boolean {
        val document: Document = Jsoup.connect("https://traodoisub.com/view/cauhinh/")
        ...
        val ids: List<Element> = document.select("td.align-middle.text-center.fs-0.white-space-nowrap.action")
        ...
        if (badges.isEmpty()) {
            ...
            if (pageText.contains("Đăng nhập", ignoreCase = true)) {
                ...
                withContext(Dispatchers.Main) {
                    val err = listOf("Hãy thêm tài khoản Facebook trước","")
        ...
        val idlist = ids.map { it.id() }
        withContext(Dispatchers.Main) {
            nickdangchay = idlist[0]
            namenickrun = badgeContents[0]
            binding.materialSpinner.setItems(badgeContents)
            binding.materialSpinner.setOnItemSelectedListener { _, _, _, item ->
                val position = badgeContents.indexOf(item.toString())
                if (position != -1) {
                    println("Phần tử 'Python' đứng ở vị trí: $position")
                    val id = idlist[position]
                    nickdangchay = id.toString()
    ...
    fun startFloatingService(context: Context) {
        val intent = Intent(context, TraoDoiSubFacebookFloating::class.java)
